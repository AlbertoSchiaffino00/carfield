...
void prepare_spatzd_boot () {
	...
	volatile uintptr_t bootaddr_addr = (uintptr_t)(CAR_FP_CLUSTER_PERIPHS_BASE_ADDR(car_spatz_cluster2) + 0x58);
	...
	volatile uintptr_t status_addr = (uintptr_t)(CAR_FP_CLUSTER_PERIPHS_BASE_ADDR(car_spatz_cluster2) + SPATZ_CLUSTER_PERIPHERAL_SPATZ_STATUS_REG_OFFSET);
	writew(1, status_addr);

	// Send IRQ
	volatile uintptr_t mbox_addr = (uintptr_t)CAR_MBOX_BASE_ADDR + MBOX_CL2_OFFSET;
...
uint32_t poll_spatzd_corestatus () {
	...
	volatile uintptr_t status_addr = (uintptr_t)(CAR_SOC_CTRL_BASE_ADDR(car_soc_ctrl) + CARFIELD_SPATZ_CLUSTER2_BUSY_REG_OFFSET);
	...
	return (uint32_t)readw(CAR_FP_CLUSTER_PERIPHS_BASE_ADDR(car_spatz_cluster2) + 0x60); //EOC register
}
