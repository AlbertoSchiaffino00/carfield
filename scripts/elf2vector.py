  ...
  def __gen_stim_header(self, filename, width):
    ...
    with open(filename, 'w') as file:
      if(self.start_addr!=0):
        start_elf = int(sorted(self.mem.keys())[0])
        file.write('#define ELF_START_ADDR %s \n' %(hex(start_elf)))
        file.write('#define ELF_BOOT_ADDR %s \n' %(hex(self.start_addr)))
        size = int(sorted(self.mem.keys())[-1]) - start_elf + 4
        file.write('#define ELF_BOOT_SIZE %s \n' %(hex(size)))
      file.write('unsigned int payload[] = {\n\t' )
      old_key = start_elf -4
      address = start_elf
      for key in sorted(self.mem.keys()):
        if int(key) - old_key != 4:
          diff = int(key) - old_key
          for i in range(0,diff//4-1):
            file.write('0x00000000')
            if key != sorted(self.mem.keys())[-1] or i != diff//4-1:
              file.write(', ')
            address += 4
        old_key = int(key)
        file.write('0x%0*X' % ( width*2, self.mem.get(key)))
        if int(address) != int(key):
          print(hex(int(address)),hex(int(key)))
        address += 4

        if key != sorted(self.mem.keys())[-1]:
          file.write(', ')
      file.write('}; \n')
